...
T_DjiReturnCode HalUart_Init(E_DjiHalUartNum uartNum, uint32_t baudRate, T_DjiUartHandle *uartHandle)
{
    ...
    /* Do not attempt to change device permissions here. Just warn if access is missing. */
    ...
    if (fp != NULL) {
        ...
        if (ret != NULL && strstr(lineBuf, "crwxrwxrwx") == NULL) {
            USER_LOG_WARN("UART device %s may lack RW permission. Add udev rule or join 'dialout' group.", uartName);
        }
    }
#else
    fp = NULL;
    if (access(uartName, R_OK | W_OK) != 0) {
        USER_LOG_WARN("UART device %s lacks RW permission. Add udev rule or run with proper group.", uartName);
    }
#endif
